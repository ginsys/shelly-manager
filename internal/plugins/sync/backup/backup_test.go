...
func TestBackupExporter_Info(t *testing.T) {
	...
	if info.Category != sync.CategoryBackup {
		t.Errorf("Expected category %v, got %v", sync.CategoryBackup, info.Category)
	}
...
func TestBackupExporter_Preview(t *testing.T) {
	...
	testData := &sync.ExportData{
		...
		Configurations: []sync.ConfigurationData{
			{DeviceID: 1},
		},
...
func TestBackupExporter_RestoreBackup(t *testing.T) {
	...
	if writeErr := os.WriteFile(backupPath, []byte("dummy backup data"), 0644); writeErr != nil {
