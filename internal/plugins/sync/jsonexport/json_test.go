...
func TestPlugin_Metadata(t *testing.T) {
	...
	if _, ok := schema.Properties["pretty"]; !ok {
		t.Error("Expected 'pretty' property in schema")
	}
...
func TestPlugin_Export_Success(t *testing.T) {
	...
	if len(envelope.Devices) != 1 {
		t.Errorf("Expected 1 device in JSON, got %d", len(envelope.Devices))
	}
	if len(envelope.Templates) != 1 {
		t.Errorf("Expected 1 template in JSON, got %d", len(envelope.Templates))
	}
...
func TestPlugin_Export_Compression(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectExt == ".json.zip" || tt.expectExt == ".json.gz" {
...
// hasDoubleSuffix checks if filename ends with double extension like .json.gz
